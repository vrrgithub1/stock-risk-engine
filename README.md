...
graph TD
    subgraph External APIs
        AlpacaWS[("Alpaca WSS (Quotes/Trades)")]
        NewsAPI["NewsAPI (REST/WS)"]
    end

    subgraph Python Async Application
        EventLoop{"Asyncio Event Loop"}
        
        subgraph Tasks
            AlpacaTask["Alpaca Listener Task"]
            NewsTask["News Fetcher Task"]
            Strategy["Strategy Manager"]
        end
        
        Queue(("Data Queue (asyncio.Queue)"))
    end

    AlpacaWS -->|WSS Stream| AlpacaTask
    NewsAPI -->|REST Poll/WS| NewsTask
    AlpacaTask -->|Push Data| Queue
    NewsTask -->|Push Data| Queue
    Queue -->|Consume| Strategy
    Strategy -->|Order| AlpacaWS
